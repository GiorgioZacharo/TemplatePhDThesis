...
3.8~\cite{LattnerMar04} infrastructure. The resulting implementation
comprises methods for the identification and analysis of the
\candidates\ (Figure~\ref{fig:flow}, box A), for the estimation of
their merit and cost (Figure~\ref{fig:flow}, box B), and for their
selection (Figure~\ref{fig:flow}, box C). Further details are provided 
...
of the conflict graph, and for subsequent selection. The control flow graph of each candidate, and
the data flow graph of each basic block are extracted so that they can be
used as input for the cost and merit calculation, according to the method
already detailed in Section~\ref{sec:cost} and~\ref{sec:merit}\\
...
each candidate is retrieved via LLVM and Clang, using a
profiling-via-instrumentation routine, which requires the generation
...
implemented HW accelerators and main memory (term $T_{Hw}^{comm}(i)$
in Section~\ref{sec:merit}), I/O requirements for each \candidate\ are
...
candidate is analysed. Every array identified accounts for extra
logical units (LUTs), contributing to the total area.

% \emph{Selection}. The selection algorithm described in
% Section~\ref{sec:algo} is implemented as a C++ program
% which receives, from the LLVM passes, the list of identified
% \candidates, their associated cost and merit, the conflict graph, and
% the value of the maximum available resources $C_{max}$. In output, it
% returns the maximum independent set that solves the \probname\
% Problem, i.e., the set of \candidates\ to be implemented in HW.
...
%Non-exact
%versions of the algorithm should of course be devised and considered,
%if larger function-call graphs required it.
