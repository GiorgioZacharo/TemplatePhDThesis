...
%then we proceed to design a local buffer that fits these needs.
\par
...
% Research papers have attempted to look into the benefits of loop unrolling in 
% HLS \cite{KurraApr07}, where they predict the optimal loop unrolling factors in HLS instances 
% by carrying out a methodology based on an estimation model. 
% On the other hand, targeting SW \cite{StephensonApr05}, 
% the authors make use of Supervised Classification, such as near neighbor (NN) classification 
% and Support Vector Machines (SVM) methods, to produce accurate predictions in optimal unrolling 
% factors. However, our suggested approach is to perform accurate predictions on the optimal loop 
% unrolling factor of loops targeted for \HLS\ by exploiting Random Forest classifiers. The 
% experimental phase supports our claim that such an approach leads to better overall 
% results and much faster convergence.
% \par
...
depend on a detailed estimation delay model of the loop body datapath and
control logic.
...
In this section, we first define the employed objective function and the performance metrics we 
considered to evaluate our framework performance. Then we introduce the LLVM analysis pass that we 
developed in order to automatically extract relevant loop features and the approach we followed to 
retrieve the area and run-time performance of HLS designs. Lastly, the supervised learning classifier 
method is detailed, which, during the training phase, gathers the data from the previous steps to 
produce a loop unrolling predictor, and, during the test phases, assigns loop unrolling factors based on loop features.


% A large set of optimizations or features exist and can be applied while synthesizing an
% accelerator using a commercial tool, such as Vivado HLS. The issues though, as mentioned
% earlier,
% The challenges that  are that:  a) they cannot be applied in an automatic fashion for each application
% and b) it requires a deep understanding of each part of an application that is going
% to be accelerated.

\par

The {\em optimal loop unrolling factor} is defined as follows.
Given a defined set of unrolling factors <1, 2, 4, 8, 16, 32, 64>, there is one for every
loop that maximizes the \textbf {Impact (I)}, given by the following formula:

$$I= \alpha  \cdot \dfrac{ (L_0 - L)} {L_0} + \beta \cdot \dfrac{ (A_0 - A)} {A_0},\ \alpha + \beta = 1$$

where $L_0$ is the latency of the loop for Loop Unrolling Factor (LUF) that is equal to one, 
meaning a rolled loop and $L$ the latency for any possible LUF from the defined set.
Respectively, $A_0$ is the area resources needs of the loop with LUF equal to one and 
...
% one
% favoring speedup, one favoring area usage and finally one that maintains a balance between 
% these two, so as to fulfill different architectural needs and explore more realistic 
% alternative scenarios.
% One approach is to target the decrease of the number of execution cycles, thus focusing on 
% increasing the speedup of an application, and a different approach would be a scenario where 
% we would like to achieve an average speedup, but maintain low the usage of HW area resources 
% in an implementation.

% Knowing the optimal loop unrolling factor for each loop of a function, besides it being a 
% non-trivial task, it is also depending on the outcome we wish to achieve. 
% One approach is to target the decrease of the number of execution cycles, thus focusing on 
% increasing the speedup of an application, and a different approach would be a scenario where 
% we would like to achieve an average speedup, but maintain low the usage of HW area resources 
% in an implementation.
% We decided to follow three different strategies, one 
% favoring speedup, one favoring area usage and finally one that maintains a balance between 
% these two, so as to fulfill different architectural needs and explore more realistic 
% alternative scenarios.\par

%After defining the optimal loop unrolling factor, 
Latency and area information are extracted
for a subset of the total number of loops of an application, for all possible unrolling factors 
in the LUF set defined above,
by using Aladdin, an estimation model 
which targets ASIC implementations. Aladdin, as mentioned in Chapter 1, allows fast, as well as 
accurate, evaluation, since 
the estimations offered are within 1\% of the ones derived from a Register-transfer level (RTL) 
implementation \cite{ShaoJul14}. By exploiting the LLVM compiler infrastructure 
\cite{LattnerMar04}, an analysis pass was developed that extracts relevant information for 
these loops, such as the loop trip count, the estimated critical path, the number of loop carried
dependencies and the number of Load and Store instructions. 
This information is used as input to a
%a Multilayer Neural Network (MNN), Support Vector Machine (SVM) and 
...
%following the formula defined above.\par
...
% predictions are generated on loops 
