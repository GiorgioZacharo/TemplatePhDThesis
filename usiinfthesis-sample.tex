...
given accelerator. $P_{SW}$ and $P_{HW}$ are measured in Watts (W) and even though 
...
% presents the idea of extending the  framework by shifting the attention 
% to candidates for acceleration that are subgraphs of the entire function call graph, instead of the function control flow graph of single functions. Furthermore the underlying system that hosts the accelerators is analyzed so that both computation and communication effort is taken into account.
% Chapter 3 is still part of ongoing research work, although preliminary results on a complex benchmark such as \htsf\ have  demonstrated improved speedup on simulated heterogeneous systems compared to profiling-only based methodologies.\par
