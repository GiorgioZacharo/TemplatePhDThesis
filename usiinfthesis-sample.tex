...
\begin{figure}[!t]
  ...
  \caption{a) An example call graph with eight nodes, and hence
          ...
          \asprobname\ are shown. A maximum independent set maximises
...
\textbf{Candidate} & \textbf{Ranking}  & \textbf{Ranking}   &  \textbf{Ranking}  \\
            & \textbf{Zynq Ultrascale+}   & \textbf{(\rseeker)} & \textbf{(gprof)} \\ \hline  \hline
  ...
  \caption{Ranking of \candidates, based on application speedup when
    ...
    strategies (\rseeker, column 3, and gprof, column 4).}
...
The quality of the choice of accelerators given by \rseeker\ was compared against 
...
to VGA ranking.
  ...
  \centering
   \resizebox{\linewidth}{!}{
    \begin{tabular}{|l||c||c|c|} 
\hline
     & \textbf{\rseeker}  & \textbf{\rseeker}   &  \textbf{\rseeker} \\
\textbf{Candidate} & \textbf{Ranking}  & \textbf{Ranking}   &  \textbf{Ranking}  \\
  ...
  \caption{Ranking of \candidates, based on merit estimation
    by \rseeker\ for varied size inputs (QCIF, column 2, CIF, column 3 and VGA, column 4). }
...
The energy saved estimation, or Merit $M()$, for a hardware accelerator $i$ is measured in nanoJoules (nJ) 
and derived by the following formulation:
...
\begin{equation*}
M(i)  = [ (Power_{SW} \times Time_{SW}) - (Power_{HW} \times Time_{HW}) ] \times n_{exec}
\end{equation*}
...
a power estimation for the hardware accelerators takes place that requires, apart from the logical
units area, the number of BRAMs, the number of DSPs and the number of arrays interfaced to every 
...
\centering
\subfigure{
  \includegraphics[width=0.7\textwidth]{figs/h264_energy.pdf}
}
\subfigure{
  \includegraphics[width=0.7\textwidth]{figs/h264_2c_energy.pdf}
}
\subfigure{  
  \includegraphics[width=0.7\textwidth]{figs/h264_3c_energy.pdf}
}
...
gprof3 profiling data strategies, varying the area constraint. Number of software CPU active cores: A) One Core 
B) Two Cores C) Four Cores.}
\label{fig:energy}
\end{figure*}
